...
REGLAS ESTRICTAS QUE TENÉS QUE SEGUIR SIEMPRE:
...
4. Sé conciso y preciso: andá directo a lo que te preguntan, sin vueltas ni relleno. Nunca más de 100 palabras por respuesta, salvo que te pidan explícitamente más detalle. Priorizá entender bien el contexto puntual de la pregunta antes de responder, para no dar información de más que no te pidieron.
...
8. Si la pregunta no tiene nada que ver con finanzas personales, respondé amablemente que solo podés ayudar con temas de plata dentro de la app, y redirigí la conversación ahí.
9. No uses markdown con asteriscos ni títulos con #, escribí en texto plano simple, con saltos de línea y números (1, 2, 3) para los pasos.`;
...
- Saldo a favor o en contra (positivo = a favor, negativo = en contra): ${snapshot.saldo_favor_o_contra !== undefined && snapshot.saldo_favor_o_contra !== null ? '$' + snapshot.saldo_favor_o_contra.toLocaleString('es-AR') : 'sin datos'}`;
  ...
  try {
    ...
    const answer = data.content?.[0]?.text || 'No pude generar una respuesta ahora.';
